"""
Tests for hardware_lock.py, ported from servo_comm_shihlin_unified
2026-09-22 alongside the fix for a real-hardware report there: a fast
keyboard tap on a JOG arrow key fires motionStart_CW/CCW (keydown)
immediately followed by motionPause (keyup), close enough together that
motionPause could arrive while motionStart_CW/CCW was still holding the
busy-lock and get rejected with 429 -- silently dropping the stop, since
nothing retries it once the key has been released. STOP_ACTIONS now wait
(bounded) for the lock instead of being rejected. The underlying race (two
independent /action requests sharing one busy-lock) is protocol-agnostic,
so this module and its tests port over unchanged.
...
class HardwareSerializedTests(unittest.TestCase):
    ...
    def test_non_json_body_is_treated_as_not_a_stop_action(self):
        ...
        /reconnect) don't send an 'action' field at all -- must not crash
        and must not wait."""
        hardware_lock._hardware_busy_lock.acquire()
        try:
