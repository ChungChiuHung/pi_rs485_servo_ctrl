...
report on servo_comm_shihlin_unified -- a fast keyboard tap on a JOG arrow
key fires motionStart_CW/CCW on keydown immediately followed by its own
keyup's motionPause, close enough together that motionPause could arrive
while motionStart_CW/CCW was still in flight and get flatly rejected with
429. Unlike a stale double-click, a dropped STOP is never retried by
anything (the key has already been released) and leaves the motor running
with no other way to stop it. So a stop-type action WAITS (bounded) for the
lock instead of being rejected outright -- "queuing" is exactly the right
behavior for a stop, since it is always correct to run as soon as the bus
is free, never "stale". Ported here even though this folder's own timing
was never separately measured, since the underlying race (two independent
/action requests sharing one busy-lock) is protocol-agnostic.
...
# the busy-lock -- see the module docstring. motionPause is the direct fix
# for the reported bug; motionCancel is the same class of action (also
...
# answering 429 -- comfortably above any single action's real duration, so
# this almost never actually triggers; it exists only to bound the wait,
# not because waiting this long is expected.
